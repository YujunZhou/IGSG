...
class Attacker(object):
    ...
    def Grad_index(self, funccall, y):
        ...
        if Dataset_type[Dataset] == 'multi':
            weight_of_embed_codes = weight_of_embed_codes.repeat(2, 1, 1)
        else:
            weight_of_embed_codes = weight_of_embed_codes.repeat(2, 1)
        weight_of_embed_codes = torch.autograd.Variable(weight_of_embed_codes, requires_grad=True).cuda()
        batch_labels = torch.LongTensor([y]).repeat(2).cuda()
        logit = self.model(weight_of_embed_codes)
        loss = self.criterion(logit, batch_labels)
        loss.backward()
        grad = weight_of_embed_codes.grad.cpu().data[0]
        grad = torch.abs(grad)
        grad[range(len(funccall)), funccall] = 0
        grad_feature, grad_cate_index = torch.max(grad, dim=1)
        self.model.zero_grad()
        self.model.eval()
        return grad_feature, grad_cate_index
    ...
    def word_paraphrase_nocombine(self, new_words, orig_prob_new, poses, category_set, y):
        ...
        batch_diagnosis_codes = torch.LongTensor(np.array(candidates))
    ...
    def attack(self, funccall, y):
        ...
        for n in range(RN):
            ...
            while robust_flag == 1 and len(arm_chain) <= budget and time_Dur <= time_limit:
                ...
                grad_set, category_set = self.Grad_index(new_words, y)
                self.model.eval()
                ...
                while robust_flag == 1 and ucb_loop <= 20 and time_Dur <= time_limit:
                    ...
                    for _ in range(budget - len(arm_chain)):
                        ...
                        cand_max, pred_max = self.word_paraphrase_nocombine(tmp_words, tmp_prob, [Feat_max],
                                                                            category_set, y)
                        tmp_arm_chain.append([Feat_max, cand_max[0]])
