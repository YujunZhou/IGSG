...
parser.add_argument('--modeltype', default='Transformer_FastBAT', type=str, help='model type')
...
parser.add_argument('--budget', default=0, type=int, help='attack_utils budget setting')
    ...
    def attack(self, funccall, y):
        ...
        while success_flag == 1:
            ...
            for visit_idx in range(self.n_con_fea, len(funccall)):
                ...
                for code_idx in range(num_avail_category[Dataset]):
                    ...
                    if Dataset in complex_categories.keys():
                        if code_idx >= complex_categories[Dataset][visit_idx - self.n_con_fea]:
                            break
            ...
            for index in range(n_batches):  # n_batches
                batch_diagnosis_codes = torch.FloatTensor(np.array(funccall_lists_all[batch_size * index: batch_size * (index + 1)]))
                ...
                with ctx_noparamgrad_and_eval(self.model):
                    t_diagnosis_codes[0] = self.adversary.perturb(t_diagnosis_codes[0], t_diagnosis_codes[1],
                                                                  torch.LongTensor([y]*t_diagnosis_codes[0].shape[0]).cuda()).detach()
