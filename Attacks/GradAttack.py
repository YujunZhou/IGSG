...
class Attacker(object):
    ...
    def attack(self, funccall, y):
        ...
        while robust_flag == 1 and len(greedy_set) < budget and time_Dur <= time_limit:
            ...
            for i in greedy_set:
                grad_norm[i] = -1
            topk_feature_index = np.argsort(grad_norm)[-N_REP:]
            candidates, candidates_change = self.find_candidate(funccall, topk_feature_index, 0)
            batch_size = 512
            ...
            if max_object > 0:
                robust_flag = 0
                break
            changed_set = self.changed_set(funccall, candidates[max_index])
