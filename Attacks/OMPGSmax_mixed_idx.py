...
class Attacker(object):
    ...
    def eval_object(self, eval_funccall, greedy_set, orig_label, greedy_set_visit_idx, query_num,
        ...
        for index in range(n_batches):  # n_batches
            self.model.eval()
            batch_diagnosis_codes = torch.FloatTensor(np.array(funccall_lists[batch_size * index: batch_size * (index + 1)]))
        ...
        for index in top_100_features:
            ...
            if Dataset in complex_categories.keys():
                if grad_cate_index_list[grad_set_index_list[index], index] >= complex_categories[Dataset][index]:
                    continue
