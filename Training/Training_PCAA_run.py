...
lrs = {'Splice': 3, 'pedec': 0.48, 'census': 0.1}
    ...
    for epoch in range(n_epoch):
        ...
        for index in range(n_batches):
            ...
            with ctx_noparamgrad_and_eval(model):
                attacker = PCAA.Attacker('', '', Dataset, Model_Type='', itermax=7, lr=lrs[Dataset], model=model)
                adv_data_discrete = attacker.generate(batch_diagnosis_codes, t_labels, 5, eval_num=10).detach()
                model.train()
                model.apply(tune_bn)
                for p in model.parameters():
                    p.requires_grad = True
                adv_data = input_process(torch.tensor(adv_data_discrete), Dataset).detach()
