...
parser.add_argument('--beta', default=1.0, type=float, help='1/lambda')
...
def trades_loss(model,
    ...
    model.eval()
    batch_size = len(x_natural[0])
    # generate adversarial example with l1 norm
    adv_con = x_natural[0]+0.001 * torch.randn(x_natural[0].shape).cuda().detach()
    adv_con = Variable(adv_con.data, requires_grad=True)
    delta_cat = 0.001 * torch.randn(x_natural[1].shape).cuda().detach()
    delta_cat = Variable(delta_cat.data, requires_grad=True)

    # Setup optimizers
    optimizer_delta = optim.Adam([delta_cat], lr=epsilon / perturb_steps * 2)

    valid_mat = valid_mat_(Dataset, model)

    for _ in range(perturb_steps):
        adv_con = Variable(adv_con.data, requires_grad=True)
        adv_cat = x_natural[1] + delta_cat
        # optimize
        optimizer_delta.zero_grad()
        with torch.enable_grad():
            loss = (-1) * criterion_kl(F.log_softmax(model(adv_con, adv_cat), dim=1),
                                       F.softmax(model(x_natural[0], x_natural[1]), dim=1))
        loss.backward()
        # renorming gradient
        grad_norms_cat = delta_cat.grad.view(batch_size, -1).norm(p=1, dim=1)
        delta_cat.grad.div_(grad_norms_cat.view(-1, 1, 1))
        # avoid nan or inf if gradient is 0
        if (grad_norms_cat == 0).any():
            delta_cat.grad[grad_norms_cat == 0] = torch.randn_like(delta_cat.grad[grad_norms_cat == 0])
        optimizer_delta.step()

        # projection
        delta_cat.data = delta_cat.data * valid_mat
        delta_cat.data.add_(x_natural[1])
        delta_cat.data.clamp_(0, 1).sub_(x_natural[1])
        delta_cat.data.renorm_(p=1, dim=0, maxnorm=epsilon)
        grad_con = adv_con.grad
        adv_con = adv_con.detach() + step_size * torch.sign(grad_con.detach())
        adv_con = torch.min(torch.max(adv_con, x_natural[0] - eps_con), x_natural[0] + eps_con)
    adv_cat = Variable(x_natural[1] + delta_cat, requires_grad=False)
    model.train()

    adv_cat = Variable(torch.clamp(adv_cat, 0.0, 1.0), requires_grad=False)
    # zero gradient
    optimizer.zero_grad()
    # calculate robust loss
    logits = model(x_natural[0], x_natural[1])
    loss_natural = F.cross_entropy(logits, y, weight=torch.FloatTensor([1, 3]).cuda())
    loss_robust = (1.0 / batch_size) * criterion_kl(F.log_softmax(model(adv_con, adv_cat), dim=1),
                                                    F.softmax(model(x_natural[0], x_natural[1]), dim=1))
